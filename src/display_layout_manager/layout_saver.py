...
class LayoutSaver:
    ...
    def find_existing_pattern_by_screen_ids(self, screen_ids: List[str], patterns: List[ConfigPattern]) -> Optional[ConfigPattern]:
    ...
    def save_current_layout(self, config_path: Path) -> SaveResult:
        ...
        try:
            ...
            # 既存パターンをチェック
            existing_pattern = self.find_existing_pattern_by_screen_ids(screen_ids, config.patterns)
