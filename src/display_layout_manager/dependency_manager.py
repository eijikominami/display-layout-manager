...
class DependencyManager:
    ...
    def check_homebrew(self) -> bool:
        ...
        if success:
            self._log(f"Homebrew 確認完了: {stdout.split()[1] if stdout.split() else 'バージョン不明'}")
            return True
        else:
    ...
    def install_gnu_grep(self) -> bool:
        ...
        if success:
            ...
            print("    export PATH=\"/opt/homebrew/opt/grep/libexec/gnubin:$PATH\"  # Apple Silicon Mac")
            print("    export PATH=\"/usr/local/opt/grep/libexec/gnubin:$PATH\"     # Intel Mac")
            return True
        else:
