...
class Logger:
    ...
    def _log(self, level: str, component: str, message: str, details: Optional[Dict[str, Any]] = None) -> None:
        """内部ログ記録メソッド"""
    ...
    def info(self, component: str, message: str, details: Optional[Dict[str, Any]] = None) -> None:
        """情報ログ"""
        self._log("INFO", component, message, details)
    
    def success(self, component: str, message: str, details: Optional[Dict[str, Any]] = None) -> None:
        """成功ログ"""
        self._log("SUCCESS", component, message, details)
    
    def warning(self, component: str, message: str, details: Optional[Dict[str, Any]] = None) -> None:
        """警告ログ"""
    ...
    def log_config_load(self, config_path: Path, pattern_count: int, success: bool, errors: Optional[list] = None) -> None:
        """設定ファイル読み込みのログ"""
        details = {
        ...
        if success:
            self.success("config", f"設定ファイル読み込み完了: {pattern_count}個のパターン", details)
        else:
            details["errors"] = errors or []
            self.error("config", "設定ファイル読み込み失敗", details)
    
    def log_display_detection(self, display_config: DisplayConfiguration, success: bool) -> None:
        """ディスプレイ検出のログ"""
        if success:
            details = {
            ...
            self.success("display", f"ディスプレイ検出完了: {len(display_config.screen_ids)}個", details)
        else:
            self.error("display", "ディスプレイ検出失敗")
    
    def log_pattern_match(self, match_result: MatchResult, current_screen_ids: list) -> None:
        """パターンマッチングのログ"""
        details = {
    ...
    def log_command_execution(self, execution_result: ExecutionResult) -> None:
        ...
        if execution_result.success:
            if execution_result.dry_run:
                ...
                self.success("command", f"コマンド実行成功: {execution_result.pattern_name}", details)
        else:
            details.update({
    ...
    def get_error_summary(self) -> list:
        ...
        return [{"component": e.component, "message": e.message, "timestamp": e.timestamp} for e in errors]
