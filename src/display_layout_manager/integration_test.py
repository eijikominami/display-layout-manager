...
class IntegrationTester:
    ...
    def test_dependency_management(self) -> TestResult:
        ...
        try:
            ...
            success = (
                all_available
                and homebrew_available
                and displayplacer_available
                and gnu_grep_available
            )
            message = "依存関係管理テスト成功" if success else "依存関係管理テスト失敗"

            execution_time = time.time() - start_time
            return TestResult(
                "dependency_management", success, message, details, execution_time
            )
    ...
    def test_config_management(self) -> TestResult:
        ...
        try:
            ...
            execution_time = time.time() - start_time
            return TestResult(
                "config_management", test_success, message, details, execution_time
            )
    ...
    def test_display_detection(self) -> TestResult:
        ...
        try:
            ...
            if success and config:
                ...
                details.update(
                    {"valid_screen_ids": valid_ids, "no_duplicates": no_duplicates}
                )
            ...
            execution_time = time.time() - start_time
            return TestResult(
                "display_detection", test_success, message, details, execution_time
            )
    ...
    def test_pattern_matching(self) -> TestResult:
        ...
        try:
            ...
            # 完全一致テスト
            exact_match_result = pattern_matcher.find_best_match(
                ["TEST-ID-1", "TEST-ID-2"], test_patterns
            )

            # 部分一致テスト
            partial_match_result = pattern_matcher.find_best_match(
                ["TEST-ID-1"], test_patterns
            )

            # 不一致テスト
            no_match_result = pattern_matcher.find_best_match(
                ["UNKNOWN-ID"], test_patterns
            )
            ...
            details = {
                "exact_match": {
                    ...
                    "pattern_name": (
                        exact_match_result.pattern.name
                        if exact_match_result.pattern
                        else None
                    ),
                ...
                "partial_match": {
                    ...
                    "pattern_name": (
                        partial_match_result.pattern.name
                        if partial_match_result.pattern
                        else None
                    ),
            ...
            # テスト成功条件
            test_success = (
                exact_match_result.matched
                and exact_match_result.match_type == "exact"
                and exact_match_result.pattern.name == "Dual Display"
            ...
            execution_time = time.time() - start_time
            return TestResult(
                "pattern_matching", test_success, message, details, execution_time
            )
    ...
    def test_command_execution(self) -> TestResult:
        ...
        try:
            ...
            execution_time = time.time() - start_time
            return TestResult(
                "command_execution", test_success, message, details, execution_time
            )
    ...
    def test_logging_system(self) -> TestResult:
        ...
        try:
            ...
            test_success = (
                len(log_entries) == 4
                and summary["by_level"].get("INFO", 0) == 1
                and summary["by_level"].get("SUCCESS", 0) == 1
                and summary["by_level"].get("WARNING", 0) == 1
                and summary["by_level"].get("ERROR", 0) == 1
            ...
            execution_time = time.time() - start_time
            return TestResult(
                "logging_system", test_success, message, details, execution_time
            )
...
"""
            ...
            details = {
                ...
                "extracted_command": (
                    extracted_command[:50] + "..." if extracted_command else ""
                ),
            }

            # テスト成功条件
            test_success = (
                pattern_name == "3_Displays_TEST-ID-_TEST-ID-_TEST-ID-"
                and extracted_command.startswith("displayplacer")
            ...
            execution_time = time.time() - start_time
            return TestResult(
                "layout_saver", test_success, message, details, execution_time
            )
