...
class PatternMatcher:
    ...
    def exact_match(self, current_screen_ids: List[str], pattern: ConfigPattern) -> bool:
        """
        完全一致チェック
        ...
        if is_match:
            ...
            self._log(f"不一致: パターン '{pattern.name}' (現在: {len(current_sorted)}個, パターン: {len(pattern_sorted)}個)")
        
        return is_match
    
    def calculate_similarity(self, current_screen_ids: List[str], pattern: ConfigPattern) -> float:
        """
        類似度を計算（Jaccard係数を使用）
    ...
    def find_best_match(self, current_screen_ids: List[str], patterns: List[ConfigPattern]) -> MatchResult:
        """
        最適なパターンを検索
    ...
    def match_display_configuration(self, display_config: DisplayConfiguration, patterns: List[ConfigPattern]) -> MatchResult:
        """
        ディスプレイ構成とパターンのマッチング
    ...
    def validate_patterns(self, patterns: List[ConfigPattern]) -> Tuple[bool, List[str]]:
        """
        パターンリストの検証
    ...
    def get_match_summary(self, match_result: MatchResult, current_screen_ids: List[str]) -> str:
        """
        マッチ結果のサマリーを生成
