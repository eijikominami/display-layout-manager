...
#### 国際化 (i18n/)
- **責務**: ユーザー向けメッセージの多言語対応
- **主要機能**:
  - 自動ロケール検出
  - メッセージカタログ管理（英語/日本語）
  - 環境変数によるオーバーライド対応
  - ログファイル（常に英語）と UI メッセージ（国際化）の分離
- **コンポーネント**:
  - **LocaleDetector**: システムロケール検出
    - `DISPLAY_LAYOUT_LANG` 環境変数をチェック
    - `LANG` 環境変数にフォールバック
    - 検出失敗時は英語をデフォルトとする
  - **MessageCatalog**: メッセージ辞書管理
    - 全てのユーザー向けメッセージを英語と日本語で保存
    - 機能カテゴリ別に整理（system、dependency、config、display、pattern、command、menu bar、error）
  - **MessageManager**: メッセージ取得とフォーマット
    - 検出されたロケールに基づいてメッセージを取得
    - パラメータ置換をサポート（例: `{count}`、`{name}`）
    - 不足キーは英語にフォールバック
- **言語サポート**:
  - **英語**: デフォルト言語、日本語以外の全てのロケールで使用
  - **日本語**: システムロケールが `ja`、`ja_JP` 等の場合に使用
- **ログファイルポリシー**:
  - ログファイルは常に英語で記録（技術的な記録のため）
  - CLI とメニューバー出力はロケールに基づいて国際化
  - これにより、デバッグとトラブルシューティングの一貫性を確保
